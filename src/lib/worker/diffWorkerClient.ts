...
export function createDiffClient(
	 ...
	 * the moment it is claimed *or* another comparison is asked for: it is the
	 * answer to the page's first ask, and a session that has moved on and come
	 * back asks again.
	 */
	...
	return {
		...
		buildTree(comparison: Comparison): Promise<DiffFileEntry> {
			...
			// or passed over for another comparison.
			adopted = null;
			if (
				claim &&
		...
		/**
		 * Reads one file's diff out of `comparison`, whose `buildTree` must have
		 * resolved. The engine reads both versions out of its cache by name, so
		 * the answer is this comparison's however other builds overlap it, and
		 * the whitespace answer is the comparison's own, so the file and its tree
		...
		getFile(
			...
			return send<FileDiff>({ type: "get-file", ...comparison, path, oldPath });
		},
